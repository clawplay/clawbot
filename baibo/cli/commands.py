...
from baibo import __logo__, __version__

app = typer.Typer(
    name="baibo",
    help=f"{__logo__} baibo - Personal AI Assistant",
...
def _enable_line_editing() -> None:
    ...
    history_file = Path.home() / ".baibo" / "history" / "cli_history"
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} baibo v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize baibo configuration and workspace."""
    from baibo.config.loader import get_config_path, save_config
    from baibo.config.schema import Config
    from baibo.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} baibo is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.baibo/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print('  2. Chat: [cyan]baibo agent -m "Hello!"[/cyan]')
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/baibo#-chat-apps[/dim]"
    )
...
I am baibo, a lightweight AI assistant.
...
def _make_provider(config):
    ...
    if not (p and p.api_key) and not model.startswith("bedrock/"):
        ...
        console.print("Set one in ~/.baibo/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from baibo.config.loader import get_data_dir, load_config
    from baibo.cron.service import CronService
    from baibo.cron.types import CronJob
    from baibo.heartbeat.service import HeartbeatService
    ...
    console.print(f"{__logo__} Starting baibo gateway on port {port}...")
    ...
    if config.memory.backend == "postgres" and config.memory.postgres.dsn:
        try:
            ...
            from baibo.agent.memory_worker import MemoryEmbeddingWorker
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from baibo.bus.events import OutboundMessage
    ...
    async def run():
        try:
            ...
            from baibo.agent.memory_factory import close_memory, initialize_memory
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show baibo runtime logs during chat"
    ),
):
    ...
    from baibo.agent.loop import AgentLoop
    from baibo.bus.queue import MessageBus
    from baibo.config.loader import load_config
    ...
    def _thinking_ctx():
        if logs:
            ...
            return nullcontext()
        return console.status("[dim]baibo is thinking...[/dim]", spinner="dots")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".baibo" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # baibo/bridge (installed)
    src_bridge = (
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall baibo")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from baibo.config.loader import get_data_dir
    from baibo.cron.service import CronService
...
def cron_add(
    ...
    from baibo.config.loader import get_data_dir
    from baibo.cron.service import CronService
    from baibo.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} baibo Status\n")
    ...
    if config_path.exists():
        from baibo.providers.registry import PROVIDERS
